...
class ActionModule(ActionNetworkModule):
    def run(self, tmp=None, task_vars=None):
        ...
        self._config_module = (
            True if module_name in ["smartcs_config", "config"] else False
        )
        ...
        if persistent_connection == "network_cli":
            ...
            provider = load_provider(smartcs_provider_spec, self._task.args)
            pc = copy.deepcopy(self._play_context)
            pc.connection = "ansible.netcommon.network_cli"
            pc.network_os = "seiko.smartcs.smartcs"
        ...
        if module_name in ["smartcs_tty_command", "tty_command"]:
